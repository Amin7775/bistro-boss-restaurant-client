...
const Testimonials = () => {
  ...
  useEffect(() => {
    axios.get("reviews.json").then((data) => setReview(data.data));
  }, []);
      ...
      <Swiper navigation={true} modules={[Navigation]} className="mySwiper">
        {review?.map((reviewItem) => (
          <SwiperSlide key={reviewItem._id}>
            <div className="mx-28 text-center mb-10 flex flex-col items-center">
              {/* rating */}
              <Rating className="mb-5" style={{ maxWidth: 220 }} value={reviewItem.rating} readOnly />
              {/* icon */}
              <svg
              className="mb-4"
              ...
              <p className="mb-1">{reviewItem.details}</p>
              <p className="text-center text-[#CD9003] text-3xl">
                {reviewItem.name}
              </p>
            </div>
          </SwiperSlide>
